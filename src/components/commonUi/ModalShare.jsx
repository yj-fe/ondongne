...
const ModalShare = ({
    ...
    return (
        <div>
            <ModalOutside onClick={ShowShareModal}>
                <ModalShareBody>
                    <ModalDiv1>공유하기</ModalDiv1>
                    <ModalDiv4>
                        {/* 링크복사 */}
                        <ModalDiv3 onClick={copyLinkUrl}>
                            <form>
                                <TextArea ref={copyLinkRef} value={window.location.href} />
                            </form>
                            <div>
                                <ImgSizeLayout _width={52} _height={52} src={Link} />
                            </div>
                            <T.Text _size={16} _width="100%" _align="center">
                                링크복사
                            </T.Text>
                        </ModalDiv3>
                        {/* 카카오톡 공유 */}
                        <ModalDiv3>
                            <button type="button" id="kakao-sharing" onClick={() => KaKaoShareButton({ itemName, description, img })}>
                                <div>
                                    <Kakao />
                                </div>
                                <T.Text _size={16} _width="100%" _align="center">
                    ...
                    <ModalButton type="button" onClick={ShowShareModal}>
                        닫기
                    </ModalButton>
                </ModalShareBody>
            </ModalOutside>
            {alert && <SimpleConfirm contents={alert.contents} onConfirmClick={alert.onConfirmClick} />}
        </div>
    );
};
