        ...
        <L.FlexCols _gap={16}>
          ...
          {item.eventType === "coupon" && (
            <C.Borderbox>
              ...
              <C.CouponUsebox
                _color={item.downloadStatus ? "#9E9E9E" : "#0B806F"}
                _bg={item.downloadStatus ? "#F5F5F5" : "#E1F3F2"}
                _dir="column"
                _gap={4}
                onClick={
                  item.downloadStatus
                    ? () => {}
                    : () => downloadHandler(item.storeCouponId, item.storeId)
                }
              >
                {item.downloadStatus ? <DownloadD /> : <DownloadC />}
                {item.downloadStatus ? "받은 쿠폰" : "쿠폰 받기"}
              </C.CouponUsebox>
            </C.Borderbox>
          )}
        </L.FlexCols>
