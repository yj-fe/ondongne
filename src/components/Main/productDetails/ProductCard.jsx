...
export function ProductCard({ item, lastRef, width = 216, isCart = true }) {
  ...
  const salePercent = disRate(
    item.price,
    item.timeSaleStatus && item.timeSale ? item.timeSale.price : item.salePrice
  );
  return (
    <L.FlexCols ref={lastRef} _gap={12} _padding={0} _width={width + "px"}>
      ...
      <L.FlexCols _gap={"4"} _padding={0}>
        {item.timeSaleStatus ? (
          ...
          item.type === "GROUP" &&
          !item.soldoutStatus && <ProductTimer endDate={item.endDate} />
        )}
        ...
        <L.FlexCols _gap={"0"}>
          {salePercent !== 0 && (
            <L.FlexRows _gap={4} _padding={0} _items="center">
              <T.Text _size={15} _weight={600} _color="red" _line="20px">
                {salePercent}%
              </T.Text>
              <T.Text
                _size={13}
          ...
          <L.FlexRows>
            <T.Text _size={16} _weight={600} _color="gray900" _line="20px">
