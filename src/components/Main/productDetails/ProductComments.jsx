...
const ProductComments = ({ id }) => {
  ...
  const getComments = useCallback(async () => {
    ...
    if (response && response.data.data) {
      ...
      setCount(Number(totalCount) + Number(response.data.data.length));
            ...
            list.map((item, i) => (
              <React.Fragment key={i}>
                ...
                <Comments
                  ...
                  child={false}
                  replyHandler={() =>
                    replyHandler(item.itemCommentId, item.nickname)
                  }
                ...
                {item.childs.length > 0 &&
                  item.childs.map((child) => (
                    <Comments
                      ...
                      child={true}
                      replyHandler={() =>
                        replyHandler(item.itemCommentId, child.nickname)
                      }
                      updateHandler={updateHandler}
                      remove={remove}
                    />
          ...
          {data.parentsId && data.replyer && (
            <ReplyBox>
              <L.FlexRows _content="flex-start" _gap={4}>
                <T.Text _size={14} _weight={600} _color="white">
                  @{data.replyer}
                </T.Text>
                <T.Text _size={14} _weight={400} _color="white">
                  님에게 답변 남기기
                </T.Text>
              </L.FlexRows>
              <button type="button" onClick={replyCancel}>
                <Cancel />
              </button>
            </ReplyBox>
          )}
      ...
      >
        ...
        <L.FlexRows _items="baseline">
          <L.FlexCols>
            ...
            <T.Text _size={15} _weight={400} _color="gray800">
              {item.contents}
            </T.Text>
            {memberId && (
              <L.FlexRows>
