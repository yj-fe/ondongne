...
function OrderManagement() {
  ...
  return (
    <CursorDiv>
      <Layout title="주문 관리" bell={false} cart={false} onBackClick={() => navigate(-1)}>
              ...
              <L.FlexCols _gap={80}>
                <L.FlexRows _gap={4} _content="space-between" _items="center">
                  ...
                  <L.FlexRows _gap={4} _content="flex-end" _width="150px" _items="center">
                    <T.Text _size={13} _weight={400} _color="gray900">
                      최근 주문 순
                    </T.Text>
                    <Down />
                  </L.FlexRows>
                ...
                {orderData.length === 0 && (
                  <L.FlexRows _height="calc(100vh - 68px)" _content="center" _item="center">
                    <T.Text _align="center" _size={15} _weight={300} _color="gray600">
                      <p>주문 내역이 없습니다.</p>
                    </T.Text>
                  </L.FlexRows>
                )}
                    ...
                    <L.FlexRows _content="space-between" _items="flex-start">
                      <L.FlexCols _gap={4}>
                        ...
                        <T.Text _size={15} _color="gray800">
                          <T.Text as="span" _size={15} _color="gray800" _weight={600}>
                            배송 방법
                          </T.Text>
                          : {item.recetiveType}
                        </T.Text>
                        <T.Text _size={15} _color="gray800">
                          ...
                          : {item.memberPhone.replace(/(\d{3})(\d{4})(\d{4})/, "$1-$2-$3")}
                        </T.Text>
                        <T.Text _size={15} _color="gray800">
                          <T.Text as="span" _size={15} _color="gray800" _weight={600}>
                            주소
                        ...
                        <T.Text _size={15} _color="gray800">
                          ...
                          : {item.deliveryContents ? item.deliveryContents : "요청사항 없음"}
                        </T.Text>
                      ...
                      <L.FlexCols _width={"15%"}>
                        <T.Text _size={15} _color="gray800" _align="right">
                          {item.orderStatus}
                        </T.Text>
                      </L.FlexCols>
                    </L.FlexRows>
                    ...
                    <L.FlexCols _gap={8}>
                      {item.orderStatus === "결제완료" && (
                        <S.Action
                          _type="bg"
                          onClick={() => {
                            statusUpdtae(item.orderId, "상품준비");
                          }}>
                          주문 확인
                        </S.Action>
                      )}
                      {(item.recetiveType === "배달" || item.recetiveType === "택배") &&
                        item.orderStatus === "상품준비" && (
                          <S.Action
                            _type="bg"
                            onClick={() => {
                              statusUpdtae(item.orderId, "배송중");
                            }}>
                            {item.recetiveType === "배달" ? "배달 진행" : "택배 진행"}
                          </S.Action>
                        )}
                      {item.recetiveType == "배달" && item.orderStatus === "배송중" && (
                        <S.Action
                          _type="bg"
                          onClick={() =>
                            navigate("/business/order/details/auth", {
                              state: {
                                id: item.orderId,
                      ...
                      {item.recetiveType == "택배" && item.orderStatus === "배송중" && (
                        <S.Action _type="bg" onClick={() => statusUpdtae(item.orderId, "배송완료")}>
                          택배 완료
                        </S.Action>
                      )}
                        ...
                        <S.Action
                          ...
                          onClick={() => {
                            return setDelOrder({
                              ...
                              confirmText: "주문 취소",
                              cancelText: "아니요",
                              onConfirmClick: () => handleOrderCancel(item.orderId),
