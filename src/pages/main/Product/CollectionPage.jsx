...
import { newStoreList, getLastGroupItemList, bestItemList } from 'service/main';
...
function CollectionPage() {
  ...
  const fetching = async () => {
    ...
    if (title === "우리동네 인기 추천") {
      return await bestItemList(local);
    }

    if (title === "공동구매 마지막 찬스") {
      return await getLastGroupItemList(local);
    }

    if (title === "우리동네 신규 입점") {
      return await newStoreList(local);
    }
              ...
              <L.Scroll _height="calc(100vh - 110px)">
                ...
                {/* =================== 있을때 =================== */}
                {
                  !loading &&
                    list.length > 0 &&
                    type === 0
                    ? <StoreListCard list={list} lastRef={ref} setData={setList} />
                    : <L.Grid _padding={0}>
                      {list.map((item, index) => (
                        <Fragment key={index}>
                          <ProductCardGrid item={item} lastRef={list.length === index + 1 ? ref : null} />
                        </Fragment>
                      ))}
                    </L.Grid>
                }
