...
import { deleteItem } from "service/bizItem";
import { CurrentNumberBox } from "components/Main/Main/MainBanner/styles";
  ...
  const getItem = async () => {
    ...
    if (response && response.data.data) {
      ...
      setItem({
        ...
        salePercent: disRate(
          resData.price,
          resData.timeSaleStatus ? resData.timeSale.price : resData.salePrice
        ),
      });
    }
  };
      ...
      <MarketTable>
        <tbody>
          <tr>
            <th>구매 형태</th>
            <td>
              {props.item.type === "GROUP" ? "공동구매 상품" : "일반 상품"}
            </td>
          </tr>
          <tr>
            <th>카테고리</th>
          ...
          <tr>
            ...
            <td>
              ...
              {props.item.deliveryPrice
                ? `, 배달비 ${numberFormat(props.item.deliveryPrice)}원`
                : ""}
              {props.item.parcelPrice
                ? `, 택배비 ${numberFormat(props.item.parcelPrice)}원`
                : ""}
            </td>
          </tr>
